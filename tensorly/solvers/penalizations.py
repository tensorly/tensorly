...
def cp_opt_balance(regs, hom_deg):
    ...
    # 3. compute scales
    scales = [
        (beta / regs[i] / hom_deg[i]) ** (1 / hom_deg[i]) for i in range(regs.shape[0])
    ]

    return scales


def tucker_implicit_sinkhorn_balancing(
    factors, core, regs, lamb_g, hom_reg, itermax=10, verbose=False
):
    ...
    for it in range(itermax):
        for mode in range(nmodes):
            core_marginal_mode = lamb_g * tl.sum(
                reg_g(core),
                axis=tuple([i for i in range(nmodes) if i != mode]),
                keepdims=True,
            )
            beta[mode] = (
            ...
            core *= (
                beta[mode].reshape(core_marginal_mode.shape)
                / core_marginal_mode
                / hom_g
            ) ** (1 / hom_g)
        if verbose:
    ...
    for mode in range(nmodes):
        for q in range(factors[mode].shape[1]):
            if regs[mode][q]:
                factors[mode][:, q] *= (
                    beta[mode][q] / regs[mode][q] / hom_reg[mode]
                ) ** (1 / hom_reg[mode])
            else:
                factors[mode][:, q] = 0

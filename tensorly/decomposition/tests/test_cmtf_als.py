...
def test_factor_match_score_3d():
    # same factors
    X_true = tl.kruskal_tensor.KruskalTensor((None, [tl.tensor([[4, 1], [0, 2]]), tl.tensor([[0, 1], [3, 5]]),
                                                     tl.tensor([[1, 1], [1, 2]])]))
    Y_true = tl.kruskal_tensor.KruskalTensor((None, [tl.tensor([[4, 1], [0, 2]]), tl.tensor([[1, 1], [1, 2]])]))
    assert_array_almost_equal(1, factor_match_score_3d(X_true, Y_true, X_true, Y_true))

    # with permutation
    X_pred = tl.kruskal_tensor.KruskalTensor((None, [tl.tensor([[1, 4], [2, 0]]), tl.tensor([[1, 0], [5, 3]]),
                                                     tl.tensor([[1, 1], [2, 1]])]))
    Y_pred = tl.kruskal_tensor.KruskalTensor((None, [tl.tensor([[1, 4], [2, 0]]), tl.tensor([[1, 1], [2, 1]])]))
    assert_array_almost_equal(1, factor_match_score_3d(X_true, Y_true, X_pred, Y_pred))


def test_cmtf_als_for_third_order_tensor():
    ...
    V = tl.tensor([[2, 0], [0, 1]])
    R = 2
    X_true = tl.kruskal_tensor.KruskalTensor([None, [A, B, C]])
    Y_true = tl.kruskal_tensor.KruskalTensor([None, [A, V]])
    X_pred, Y_pred = cmtf_als_for_third_order_tensor(X_true, Y_true, R)
    ...
    X_pred, Y_pred = cmtf_als_for_third_order_tensor(X, Y, R)
    assert_array_less(0.99 ** 4, factor_match_score_3d(X_true, Y_true, X_pred, Y_pred))

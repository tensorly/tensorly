...
class PyTorchBackend(Backend, backend_name="pytorch"):
    ...
    def _from_torch(data, dtype, device, requires_grad):
        ...
        return torch.tensor(data.clone(), dtype=dtype, device=device, requires_grad=requires_grad)
    ...
    def tensor(data, dtype=None, device=None, requires_grad=None):
        ...
        if isinstance(data, torch.Tensor):
            ...
            return PyTorchBackend._from_torch(data, dtype=dtype, device=device, requires_grad=requires_grad)
        elif isinstance(data, np.ndarray):
            # Input is a numpy array
            return PyTorchBackend._from_numpy(data, dtype=dtype, device=device, requires_grad=requires_grad)
        else:
            return PyTorchBackend._from_python(data, dtype=dtype, device=device, requires_grad=requires_grad)

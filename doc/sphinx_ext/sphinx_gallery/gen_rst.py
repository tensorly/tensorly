...
def scale_image(in_fname, out_fname, max_width, max_height):
    ...
    if os.environ.get("SKLEARN_DOC_OPTIPNG", False):
        try:
            ...
            logger.warning("Install optipng to reduce the size of the generated images")
...
def save_thumbnail(image_path_template, src_file, file_conf, gallery_conf):
    ...
    if not isinstance(thumbnail_number, int):
        raise TypeError("sphinx_gallery_thumbnail_number setting is not a number.")
    ...
    if src_file in gallery_conf["failing_examples"]:
        ...
        default_thumb_file = gallery_conf.get("default_thumb_file", default_thumb_file)
...
def generate_dir_rst(src_dir, target_dir, gallery_conf, seen_backrefs):
    ...
    with codecs.open(os.path.join(src_dir, "README.txt"), "r", encoding="utf-8") as fid:
        fhindex = fid.read()
    ...
    sorted_listdir = [
        fname for fname in sorted(os.listdir(src_dir)) if fname.endswith(".py")
    ]
    entries_text = []
    computation_times = []
    ...
    for fname in iterator:
        ...
        this_entry = (
            _thumbnail_div(build_target_dir, fname, intro)
            + """
...
.. toctree::
   ...
   /%s\n"""
        ...
        if gallery_conf["backreferences_dir"]:
            write_backreferences(seen_backrefs, gallery_conf, target_dir, fname, intro)
...
def handle_exception(exc_info, src_file, block_vars, gallery_conf):
    ...
    logger.warning("%s failed to execute correctly: %s", src_file, formatted_exception)
...
def execute_code_block(
    src_file, code_block, lineno, example_globals, block_vars, gallery_conf
):
    ...
    try:
        ...
        except_rst = handle_exception(
            sys.exc_info(), src_file, block_vars, gallery_conf
        )
        code_output = "\n{0}\n\n\n\n".format(except_rst)
...
def generate_file_rst(fname, target_dir, src_dir, gallery_conf):
    ...
    for blabel, bcontent, lineno in script_blocks:
        if blabel == "code":
            code_output, rtime = execute_code_block(
                src_file, bcontent, lineno, example_globals, block_vars, gallery_conf
            )

            time_elapsed += rtime

            if not file_conf.get(
                "line_numbers", gallery_conf.get("line_numbers", False)
            ):
                lineno = None
    ...
    with codecs.open(
        os.path.join(target_dir, base_image_name + ".rst"), mode="w", encoding="utf-8"
    ) as f:
        example_rst += (
            "**Total running time of the script:**"
            " ({0: .0f} minutes {1: .3f} seconds)\n\n".format(time_m, time_s)
        )
        example_rst += CODE_DOWNLOAD.format(fname, fname.replace(".py", ".ipynb"))

...
def test_extract_intro():
    ...
    assert (
        result == "This is the description of the example which goes on and on, Óscar"
    )  # noqa
...
def gallery_conf():
    ...
    gallery_conf.update(examples_dir=tempfile.mkdtemp(), gallery_dir=tempfile.mkdtemp())
...
def test_pattern_matching(gallery_conf, log_collector):
    ...
    for fname in fnames:
        with codecs.open(
            os.path.join(gallery_conf["examples_dir"], fname),
            mode="w",
            encoding="utf-8",
        ) as f:
            f.write("\n".join(CONTENT))

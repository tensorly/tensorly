...
'backreferences_dir': False\n"""
        ...
        gallery_conf["backreferences_dir"] = os.path.join("modules", "generated")
        logger.warning(
...
def get_subsections(srcdir, examples_dir, sortkey):
    ...
    subfolders = [
        subfolder
        for subfolder in os.listdir(examples_dir)
    ...
    for examples_dir, gallery_dir in workdirs:
        ...
        if not os.path.exists(os.path.join(examples_dir, "README.txt")):
            raise FileNotFoundError(
                "Main example directory {0} does not "
        ...
        with codecs.open(
            os.path.join(gallery_dir, "index.rst"), "w", encoding="utf-8"
        ) as fhindex:
            # :orphan: to suppress "not included in TOCTREE" sphinx warnings
            fhindex.write(":orphan:\n\n" + this_fhindex)

            for subsection in get_subsections(
                app.builder.srcdir, examples_dir, gallery_conf["subsection_order"]
            ):
...
def sumarize_failing_examples(app, exception):
    ...
    examples_expected_to_fail = failing_examples.intersection(expected_failing_examples)
